...
class Rucio :
    ...
    def addReplicaRule(self, destRSE, group):
        ...
        if destRSE:
            try:
                rule = self.rulesClient.add_replication_rule([{"scope":self.myscope,"name":group}],copies=1, rse_expression=destRSE, grouping='ALL', account=self.account, purge_replicas=True, asynchronous=True)
    ...
    def outdated_register_replica(self, filemds, dest_RSE, org_RSE):
        ...
        '''for i in range(0,100):
            while True:
                try:
                    # do stuff
                    rule = self.addReplicaRule_noasync(dest_RSE, group=carrier_dataset)
                    if rule != None :
                        rule_child = rule 
                except :
                    continue
        ...
        for i in range(0,1000):
            ...
            try:
                # do stuff
                rule = self.addReplicaRule(org_RSE, group=carrier_dataset)
                if rule != None :
                    rule_parent = rule
                print(rule_parent)
                break
...
class Look_for_Files :
    ...
    def scrap_through_dir(self, path) : 
        ...
        #print("*-Listin files from url : %s" % path)
        all_files = []
        ...
        for dest in r1.destRse :
            ...
            for n in range(0,len(listOfFiles)):
                ...
                if check != False : ## needs to be changed to False
                    ...
                    print('Replication performed between ',dest, group['container_2'])
                    main_rule = r1.addReplicaRule(dest, group['container_2'])
